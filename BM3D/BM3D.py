...
def bm3d_r():
    img = cv2.imread(data_path.image_r_path, cv2.IMREAD_GRAYSCALE)
    ...
    cv2.imwrite("/home/qianrui/BM3D/data/Basic3.jpg", Basic_img)
...
#   print "The PSNR between the two img of the First step is %f" % psnr
    ...
    cv2.imwrite(data_path.image_r_path, Final_img)
...
#    print "The PSNR between the two img of the Second step is %f" % psnr
    ...
    print "The total Processing R-channel time is %f s" % time

def bm3d_g():
    img = cv2.imread(data_path.image_g_path, cv2.IMREAD_GRAYSCALE)
...
#    print "The PSNR between the two img of the First step is %f" % psnr
    ...
    time = (e3 - e2) / cv2.getTickFrequency()
    print "The Processing time of the Second step is %f s" % time
    cv2.imwrite(data_path.image_g_path, Final_img)
#    psnr = PSNR.PSNR(img, Final_img)
#    print "The PSNR between the two img of the Second step is %f" % psnr
    time = (e3 - e1) / cv2.getTickFrequency()   
    print "The total Processing G-channel time is %f s" % time

def bm3d_b():
    img = cv2.imread(data_path.image_b_path, cv2.IMREAD_GRAYSCALE)
...
#    print "The PSNR between the two img of the First step is %f" % psnr
    ...
    time = (e3 - e2) / cv2.getTickFrequency()
    print "The Processing time of the Second step is %f s" % time
    cv2.imwrite(data_path.image_b_path, Final_img)
#    psnr = PSNR.PSNR(img, Final_img)
#    print "The PSNR between the two img of the Second step is %f" % psnr
    time = (e3 - e1) / cv2.getTickFrequency()   
    print "The total Processing B-channel time is %f s" % time

def refresh():
    img = cv2.imread(data_path.original_image_path)
    row = img.shape[0]
    col = img.shape[1]
    img_r = cv2.imread(data_path.image_r_path, cv2.IMREAD_GRAYSCALE)
    img_g = cv2.imread(data_path.image_g_path, cv2.IMREAD_GRAYSCALE)
    img_b = cv2.imread(data_path.image_b_path, cv2.IMREAD_GRAYSCALE)
    for i in range(row):
        for j in range(col):
            if mask.mask[i][j] == 1:
                img_r[i][j] = img[i][j][0]
